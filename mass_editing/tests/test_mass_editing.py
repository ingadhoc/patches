...
class TestMassEditing(common.TransactionCase):
    ...
    def _create_mass_editing(self, model, fields):
        ...
        mass = self.mass_object_model.create({
            ...
            'mass_editing_model_id': model.id,
    ...
    def test_onchange_model(self):
        """Test whether onchange mass_editing_model_id returns mass_editing_model_id in list"""
        new_mass = self.mass_object_model.new({'mass_editing_model_id': self.user_model.id})
        new_mass._onchange_mass_editing_model_id_domain()
        model_list = ast.literal_eval(new_mass.model_list)
        self.assertTrue(self.user_model.id in model_list,
                        'Onchange model list must contains mass_editing_model_id.')
